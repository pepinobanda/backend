...
class ClienteDAO {
    public async lista() {
        const result  = await pool.then(async (connection) => {
            return await connection.query("SELECT u.cveCliente, u.nombre, u.apellidos, u.cveRegistro, u.tipoCliente, r.descripcion as rol FROM cliente u JOIN rol r ON r.cveRol = u.tipoCliente ORDER BY u.nombre, u.apellidos ASC");
        });
